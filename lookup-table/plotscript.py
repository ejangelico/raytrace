...
def getVelocityDistribution(rays, time):
	nsum = 0
	rvs = []
	for r in rays:
		if(r.isAbsorbed()):
			continue
		rp, rt, rv, n = r.findLastReflection(time)
		nsum += n
		rvs.append(rv.normalize())

	nave = nsum/(float(len(rvs)))

	return (rvs, nave)

#plots the vx,vy,vz velocity distribution
#of all rays at a certain timeslice
def plotVelocityDistribution(rays, time):
	rvs, nave = getVelocityDistribution(rays, time)

	vx = [v.getX() for v in rvs]
	vy = [v.getY() for v in rvs]
	vz = [v.getZ() for v in rvs]
	...
	ax.set_ylim([-1, 1])
	ax.set_zlim([-1, 1])
	ax.scatter(vx, vy, vz, c='k', marker='o')
	...
	#titles
	ax.set_title("Velocity distribution at time " + str(time) + "ns,\n average number of reflections N_ave = " + str(nave), fontsize=26, y=1.09)
	ax.set_xlabel("Vx", fontsize=26)
	ax.set_ylabel("Vy", fontsize=26)
	ax.set_zlabel("Vz", fontsize=26)

	#plt.show()


def animateVelocityDistribution(rays, window, frames):
	fig = plt.figure(figsize=(10,10))
	ax = fig.gca(projection='3d')
	...
	pset = ax.scatter([],[],[], c='k', marker='o')

	def driver(time, rays):
		rvs, nave = getVelocityDistribution(rays, time)
		vx = [v.getX() for v in rvs]
		vy = [v.getY() for v in rvs]
		vz = [v.getZ() for v in rvs]
		pset._offsets3d = (vx, vy, vz)
		ax.set_title("Velocity distribution at time " + "{0:.3f}".format(time) + "ns,\n average number of reflections N_ave = " + "{0:.2f}".format(nave), fontsize=26, y=1.09)
		return (pset,ax)


	
	anim = FuncAnimation(fig, driver,frames=np.linspace(window[0], window[1], frames),fargs=[rays],interval=200)
	anim.save('cube_centeredVertex45.gif', dpi=80, writer='imagemagick', fps=30)

#calculate the avg number of 
#reflections after "time" of 
#propagation
def getAvgN(rays, time):
	nsum = 0
	for r in rays:
		rp, rt, rv, n = r.findLastReflection(time)
		nsum += n

	return nsum/(float(len(rays)))
...
def plotReflectionPoints(detector, rays):
	print "Drawing detector and event"
	...
	plt.axis('off')
	regs = detector.getRegions()
	[r.plotReflectionPoints(ax) for r in rays]
	#plt.show()
	plt.savefig("detectorPics/cube_vertex45_refpoints.png", bbox_inches='tight')

	


(eventrays, detector) = loadData("data/cube_vertex(0,0,1)(0,1,1)_100ns.p")
#spreadrays = spaceTimeSpread(eventrays, 7e-4, 7e-4, 7e-4, 0.3)
#qerays = qeReduce(spreadrays, 20)
#plotReflectionPoints(detector, eventrays)
timewindow = [0, 100]
frames = 300
animateVelocityDistribution(eventrays, timewindow, frames)
sys.exit()
