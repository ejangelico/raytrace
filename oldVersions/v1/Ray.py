...
class Ray:
	...
	def setIntersections(self, inters):
		if(self.intersectPoints != None and len(self.intersectPoints) > 100):
			print "Before = " + str(len(self.intersectPoints))
		self.intersectPoints = inters
		if(self.intersectPoints != None and len(self.intersectPoints) > 100):
			print "After = " + str(len(self.intersectPoints))
			print "***"
	...
	def checkIntersection(self, timestep):
		...
		if(reg.getMat() == 'mirror'):
			...
			if(np.arccos(self.v*n) > np.pi/2.0):
				n = n.scale(-1)
	...
	def plotRay(self, ax):
		...
		#	ax.scatter(p[0].getX(), p[0].getY(), p[0].getZ(), c='k', marker='x')
